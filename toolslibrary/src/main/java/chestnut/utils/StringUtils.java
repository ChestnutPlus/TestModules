...
public class StringUtils {
     ...
     * @param src   源字符
     * @param flag  sub的标志位
     * @param beginIndexFlag    开始的index    1-N
     * @param endIndexFlag      结束的index    1-N
     * @return
     */
    public static String getSubStr(String src,char flag,int beginIndexFlag,int endIndexFlag) {
        String temp = src;
        int a = 0;
        int b;
        int p = 0;
        for (int i = 0; i < endIndexFlag; i++) {
            int y = temp.indexOf(String.valueOf(flag))+1;
            p += y;
            temp = temp.substring(y);
            if (beginIndexFlag-1==i) {
                a = p-1;
            }
        }
        b = p-1;
        return String.valueOf(src.subSequence(a+1,b));
    }
}